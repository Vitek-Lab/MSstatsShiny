  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$choice1 <- renderUI({
        selectInput(session$ns("group1"), "Group 1", choices())
      })
      
      output$choice2 <- renderUI({
        selectInput(session$ns("group2"), "Group 2", choices())
      })
      
      output$choice3 <- renderUI({
        selectInput(session$ns("group3"), "", choices())
      })
      
      output$comp_name <- renderUI({
        textInput(session$ns("comp_name"), label = "Comparison Name", value = "")
      })
      
      output$weights <- renderUI({
        lapply(1:length(choices()), function(i) {
          list(
            numericInput(session$ns(paste0("weight", i)), label = choices()[i], value = 0)
          )
        })
      })
      ...
      output$WhichComp <- renderUI({
        selectInput(session$ns("whichComp"),
      ...
      output$WhichProt <- renderUI({
        selectInput(session$ns("whichProt"),
                    label = h4("which protein to plot"), 
                    unique(get_data()[[1]]))
      })
      
      output$WhichProt1 <- renderUI({
        selectizeInput(session$ns("whichProt1"),
                       label = h4("which protein to plot"), 
                       c("", unique(get_data()[[1]])))
      })
      ...
      matrix_build <- eventReactive(input$submit | input$submit1 | input$submit2 | input$submit3, {
        ...
        if (input$def_comp == "custom") {
          ...
          comp_list$dList <- unique(c(isolate(comp_list$dList), paste(input$group1, "vs", input$group2, sep = " ")))
      ...
      SignificantProteins <- eventReactive(input$calculate, {
        if (loadpage_input()$BIO == "PTM") {
          ...
          sig_unadj <- data_comp$PTM.Model[data_comp$PTM.Model$adj.pvalue < input$signif, ]
          sig_prot <- data_comp$PROTEIN.Model[data_comp$PROTEIN.Model$adj.pvalue < input$signif, ]
          sig_adj <- data_comp$ADJUSTED.Model[data_comp$ADJUSTED.Model$adj.pvalue < input$signif, ]
          significant <- list(PTM.Model = sig_unadj,
                              PROTEIN.Model = sig_prot,
                              ADJUSTED.Model = sig_adj)
        } else if (loadpage_input()$DDA_DIA == "TMT") {
          data_comp <- data_comparison()
          significant <- data_comp$ComparisonResult[data_comp$ComparisonResult$adj.pvalue < input$signif, ]
        } else {
          data_comp <- data_comparison()
          significant <- data_comp$ComparisonResult[which(data_comp$ComparisonResult$adj.pvalue < input$signif), ]
          print(significant)
        }
        return(significant)
      })
      ...
      assumptions1 <- function(saveFile3, protein) {
        if (input$whichProt1 != "") {
          ...
          plots <- modelBasedQCPlots(data = data_comparison(), 
      ...
      observeEvent(input$calculate, {
        output$code.button <- renderUI({
          downloadButton(session$ns("download_code"), 
      ...
      output$matrix <- renderUI({
        tagList(
          ...
          br(),
          textOutput(session$ns("message")),
          br(),
          if (is.null(contrast$matrix)) {
            ""
          } else {
            dataTableOutput(session$ns("table"))
          }
        )
      })
      ...
      output$table_results <- renderUI({
        ...
        if (is.null(significant)) {
          ...
          tagList(
            tags$br(),
            h2("Results"),
            h5("There are ", textOutput(session$ns("number"), inline = TRUE), "significant proteins"),
            tags$br(),
            dataTableOutput(session$ns("significant")),
            downloadButton(session$ns("download_compar"), "Download all modeling results"),
            downloadButton(session$ns("download_signif"), "Download significant proteins")
          )
        }
      })
      ...
      output$adj_table_results <- renderUI({
        ...
        if (is.null(significant$ADJUSTED.Model)) {
          ...
          tagList(
            ...
            h5("There are ", textOutput(session$ns("number_adj"), inline = TRUE), "significant PTMs"),
            tags$br(),
            dataTableOutput(session$ns("adj_significant")),
            downloadButton(session$ns("download_compar_adj"), "Download all modeling results"),
            downloadButton(session$ns("download_signif_adj"), "Download significant PTMs")
          )
        }
      })
      ...
      output$unadj_table_results <- renderUI({
        ...
        if (is.null(significant$PTM.Model)) {
          ...
          tagList(
            ...
            h5("There are ", textOutput(session$ns("number_unadj"), inline = TRUE), "significant PTMs"),
            tags$br(),
            dataTableOutput(session$ns("unadj_significant")),
            downloadButton(session$ns("download_compar_unadj"), "Download all modeling results"),
            downloadButton(session$ns("download_signif_unadj"), "Download significant PTMs")
          )
        }
      })
      ...
      output$prot_table_results <- renderUI({
        ...
        if (is.null(significant$PTM.Model)) {
          ...
          tagList(
            ...
            h5("There are ", textOutput(session$ns("number_prot"), inline = TRUE), "significant proteins"),
            tags$br(),
            dataTableOutput(session$ns("prot_significant")),
            downloadButton(session$ns("download_compar_prot"), "Download all modeling results"),
            downloadButton(session$ns("download_signif_prot"), "Download significant proteins")
          )
        }
      })
      ...
      observeEvent(input$viewresults, {
        if (loadpage_input()$BIO == "PTM") {
          ...
          op <- plotOutput(session$ns("comp_plots"))
        } else {
          output$comp_plots <- renderPlotly({
            group_comparison(FALSE, FALSE)
          })
          op <- plotlyOutput(session$ns("comp_plots"), height = input$height)
        }
        
        insertUI(
          selector = paste0("#", session$ns("comparison_plots")),
          ui = tags$div(
            op,
            conditionalPanel(
              condition = "input['statmodel-typeplot'] == 'VolcanoPlot' && input['loadpage-DDA_DIA']!='TMT'",
              h5("Click on plot for details"),
              verbatimTextOutput(session$ns("info2"))
            ),
            conditionalPanel(
              condition = "input['statmodel-typeplot'] == 'Heatmap'",
              sliderInput(session$ns("height"), "Plot height", value = 500, min = 200, max = 1300, post = "px")
            )
          )
        )
      })
      ...
      output$verify <- renderUI({
        tagList(
          plotOutput(session$ns("assum_plots"), width = "800px", height = "600px"),
          conditionalPanel(
            condition = "input['statmodel-whichProt1'] != ''",
            actionButton(session$ns("saveone1"), "Save this plot"),
            bsTooltip(id = session$ns("saveone1"), 
                      title = "Open plot as pdf.  Popups must be enabled", 
                      placement = "bottom", 
                      trigger = "hover"),
            actionButton(session$ns("saveall1"), "Save all plots"),
            bsTooltip(id = session$ns("saveall1"), 
                      title = "Open pdf of all plots.  Popups must be enabled", 
                      placement = "bottom", 
                      trigger = "hover")
          )
        )
      ...
      output$download_code <- downloadHandler(
        ...
        content = function(file) {
          writeLines(paste(data_comparison_code(), sep = ""), file)
        }
      )
