...
prepare_turnover_for_dose_response <- function(ratios, add_zero_timepoint = FALSE, increasing = TRUE) {
  ...
  # Carry per-peptide quality weights through when present (added by
  # calculatePeptideWeights) so they can be passed to doseResponseFit /
  # visualizeResponseProtein via their `weights` argument, row-aligned with
  ...
  if (add_zero_timepoint) {
    ...
    if (nrow(needs_zero) > 0) {
      ...
      # Synthetic anchor points are a known biological constraint, not a noisy
      # measurement, so they are fully trusted (weight 1).
